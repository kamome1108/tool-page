...
export default function CharacterCounterClient({ locale }: { locale: string }) {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-4">
                        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
                            <h2 className="text-gray-500 text-sm uppercase tracking-wider mb-6 font-semibold">{t('ui.statistics')}</h2>

                            <div className="space-y-6">
                                <div>
                                    <div className="text-4xl font-bold text-green-600">{stats.chars}</div>
                                    <div className="text-gray-600 text-sm mt-1">{t('ui.characters')}</div>
                                </div>

                                <div>
                                    <div className="text-2xl font-bold text-emerald-600">{stats.charsNoSpace}</div>
                                    <div className="text-gray-600 text-sm mt-1">{t('ui.withoutSpaces')}</div>
                                </div>

                                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                                    <div>
                                        <div className="text-xl font-bold text-gray-900">{stats.lines}</div>
                                        <div className="text-gray-600 text-xs mt-1">{t('ui.lines')}</div>
                                    </div>
                                    <div>
                                        <div className="text-xl font-bold text-gray-900">{stats.paragraphs}</div>
                                        <div className="text-gray-600 text-xs mt-1">{t('ui.paragraphs')}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
